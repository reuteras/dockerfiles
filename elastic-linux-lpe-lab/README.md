...
To test detection you can for example use the Copy Fail provided code in the *pocs* directory.
If you select this test follow the instructions below to install an older and vulnerable kernel.

Another example is to use the newer exploit chain using the Fragnesia primitive via Open vSwitch.
Read the post [The skb that wasn't freed - the Fragnesia primitive via Open vSwitch](https://blog.doyensec.com/2026/09/17/ovs.html) and get
the POC from [ovs-pagecache-write](https://github.com/doyensec/ovs-pagecache-write) on GitHub.
...
If the automated scripts fail or you need to customize the setup, see
[docs/MANUAL.md](docs/MANUAL.md) for the manual equivalents (generating the
env file, starting Elasticsearch/Kibana, configuring Fleet Server, creating
the endpoint policy, and enrolling a Linux VM by hand).
